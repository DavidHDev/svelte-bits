...
`;
	...
	function autoBind(self: object, { include, exclude }: AutoBindOptions = {}) {
		...
		for (const [object, key] of getAllProperties(Object.getPrototypeOf(self))) {
			if (key === 'constructor' || !filter(key)) continue;
			const descriptor = Reflect.getOwnPropertyDescriptor(object, key);
			if (descriptor && typeof descriptor.value === 'function') {
				Object.defineProperty(self, key, {
					configurable: true,
					value: descriptor.value.bind(self)
				});
			}
		}
		return self;
...
<div bind:this={containerRef} class="relative z-[2] h-full w-full overflow-hidden {className}">
